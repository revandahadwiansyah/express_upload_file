...
DecompressZip.prototype.extractFiles = function (files, options, results) {
    var promises = [], fileBuckets = [];
    var self = this;

    results = results || [];
    var fileIndex = 0;
    files.forEach(function (file) {
        file.filename = (options.timestamps)? `${Date.now()}_${file.filename}` : file.filename;
            file.path = (options.timestamps)? `${Date.now()}_${file.path}` : file.path;
